...
describe('transformStringWith* from various sources', () => {
	const testContent = utils.readCustom(
		'./test/custom.css',
		'./test/custom.js',
	...
	test.each(testSources)('%s', (description, testParams) => {
		...
		return testContent.then(content => {
			const testResult = resultUtil(testSource, content[ description ])
...
describe('transformStringWith* from a json file', () => {
	const testContent = utils.readCustomFromJsonFile('./test/custom.json')
	const testSources = Object.entries({
		customMedia: {
			source: 'all and (--mq-a)',
			expect: 'all and (max-width: 30em),all and (max-height: 30em)',
...
describe('readCustom imports different formats', () => {
	...
	test.each(testSources)('%s', (description, testExpect) => {
		...
		return testContent.then(content => {
			return Promise.all([
				utils.writeCustom(content, path),
				utils.readFile(path),
			]).then(([,fileContent]) => {
				return expect(fileContent.replace(/\s/g, '')).toBe(testExpect.replace(/\s/g, ''))
			}).catch(error => {
				console.error('Something broke:', error)
				return false
			})

			// return Promise.resolve(utils.writeCustom(content, path)).then(() => {
			// 	return Promise.resolve(utils.readFile(path)).then(fileContent => {
			// 		return expect(fileContent.replace(/\s/g, '')).toBe(testExpect.replace(/\s/g, ''))
			// 	})
			// })

			// return Promise.resolve(read).then(fileContent => {
			// 	return expect(fileContent.replace(/\s/g, '')).toBe(testExpect.replace(/\s/g, ''))
			// })
		})
	})
